...
test('Load palette', async t =>{
    ...
    var rect = await page.getRect(page.setGitToken);
    await showNoteBox('To access the repositories, create a GitHub access token with the correct permissions', rect, 'left', 1.5);
    ...
    await showNoteBox('Click the repository containing the palette', rect, 'left', 1.0);
    ...
    await showNoteBox('The list of nodes in the palette are accessed here', rect, 'right', 1.2);
...
test('Graph creation', async t =>{
    ...
    await showNoteBox('Memory drops are used to avoid moving the data wherever possible', rect, 'above', 1.5);
...
test('Saving a graph', async t =>{
    ...
    // Note
    var rect = await page.getRect(page.setGitToken);
    await showNoteBox('You need to enter a GitHub access token with all \"repo\" permissions for the added repository', rect, 'left', 1.5);
