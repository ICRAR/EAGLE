...
test('Update components', async t =>{
    ...
    await t.expect(obj2.nodeDataArray[2].fields[5].name).eql("streaming", {timeout:3000});
    await t.expect(obj2.nodeDataArray[2].fields[6].name).eql("bufsize", {timeout:3000});
    await t.expect(obj2.nodeDataArray[2].fields[7].name).eql("input_error_threshold", {timeout:3000});
