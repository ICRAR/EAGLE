...
import type { SettingsGroup } from './Setting';
...
export class Eagle {
    ...
    loadFileFromUrl = async(fileType: Eagle.FileType): Promise<void> => {
        ...
        try {
            void Repositories.selectFile(new RepositoryFile(new Repository(Repository.Service.Url, "", "", false), "", url));
        } catch(error){
            console.error(error);
        }
    }
    ...
    nodeDropLogicalGraph = (_eagle : Eagle, event: JQuery.TriggeredEvent) : void => {
        ...
        for (const sourceComponent of sourceComponents){
            void this.addNodeToLogicalGraph(sourceComponent, null, Eagle.AddNodeMode.Default);
