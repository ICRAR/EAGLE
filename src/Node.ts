...
export class Node {
    ...
    setGroupStart = (value: boolean) : Node => {
        // get group start field
        let groupStartField = this.getFieldByDisplayText(Daliuge.FieldName.GROUP_START);
    ...
    setGroupEnd = (value: boolean) : Node => {
        // get group end field
        let groupEndField = this.getFieldByDisplayText(Daliuge.FieldName.GROUP_END);
    ...
    getLocalMultiplicity = () : number => {
        if (this.isMKN()){
            const k = this.getFieldByDisplayText(Daliuge.FieldName.K);
    ...
    static fromOJSJson(nodeData : any, errorsWarnings: Errors.ErrorsWarnings, isPaletteNode: boolean) : Node {
        let id: NodeId | null = Node.determineNodeId(nodeData);
    ...
    static isValid(node: Node, location: Eagle.FileType) : void {
        ...
        for (const child of children.values()){
            ...
            if (parent === null || parent.getId() !== node.getId()){
        ...
        const embed = node.embed();
        if (embed !== null){
            const inputApplication = embed.inputApplication();
            const outputApplication = embed.outputApplication();

            if ((inputApplication !== null && inputApplication.getId() !== node.getId()) && (outputApplication !== null && outputApplication.getId() !== node.getId())){
        ...
        const inputApplication = node.inputApplication();
        if (inputApplication !== null){
            const embed = inputApplication.embed();
            if (embed !== null && embed.getId() !== node.getId()){
        ...
        const outputApplication = node.outputApplication();
        if (outputApplication !== null){
            const embed = outputApplication.embed();
            if (embed !== null && embed.getId() !== node.getId()){
        ...
        if (node.category() === Category.Memory || node.category() === Category.SharedMemory){
            const pydataField = node.getFieldByDisplayText(Daliuge.FieldName.PYDATA);
            ...
            if (hasInputEdge && hasPydataValue){
                ...
                const issue: Errors.Issue = Errors.ShowFix(message, function(){Utils.showNode(eagle, location, node)}, function(){if (typeof pydataField === 'undefined'){return} if (pydataField.getValue() === ""){pydataField.setValue(Daliuge.DEFAULT_PYDATA_VALUE);}}, "Replace empty pydata with default value (" + Daliuge.DEFAULT_PYDATA_VALUE + ")");
                node.issues().push({issue:issue,validity:Errors.Validity.Warning})
            }
        }
