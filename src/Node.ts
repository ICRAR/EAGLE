...
export class Node {
    ...
    isValid(selectedLocation:Eagle.FileType) : void {
        const eagle = Eagle.getInstance()
        const node = this;
        const errorsWarnings = this.errorsWarnings

         // check that node has modern (not legacy) category
        if (this.getCategory() === Category.Component){
        ...
        if (node.hasInputApplication()){
            node.getInputApplication().isValid(selectedLocation);
        }
        if (node.hasOutputApplication()){
            node.getOutputApplication().isValid(selectedLocation);
        }
    ...
    // static isValid(eagle: Eagle, node: Node, selectedLocation: Eagle.FileType, showNotification : boolean, showConsole : boolean) : Eagle.LinkValid {
    //     // check that node has modern (not legacy) category
    ...
    private static _checkForField(eagle: Eagle, node: Node, field: Field, errorsWarnings: Errors.ErrorsWarnings) : void {
        ...
        if (existingField === null){
            ...
            if (existingField.getParameterType() !== field.getParameterType()){
                ...
                errorsWarnings.errors.push(Errors.ShowFix(message, function(){Utils.showNode(eagle, node.getId());}, function(){Utils.fixFieldParameterType(eagle, node, existingField, field.getParameterType())}, "Switch type of field to '" + field.getParameterType()));
            }
        }
    }
}
