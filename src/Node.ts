...
export class Node {
    ...
    static fromOJSJson = (nodeData : any, errors: string[], generateKeyFunc: () => number) : Node => {
        ...
        if (typeof nodeData.text !== 'undefined'){
            ...
            errors.push("Node " + nodeData.key + " has undefined text " + nodeData + "!");
        }
        ...
        if (nodeData.inputAppName !== undefined && nodeData.inputAppName !== ""){
            if (!Eagle.getCategoryData(category).canHaveInputApplication){
                errors.push("Attempt to add inputApplication to unsuitable node: " + category);
            } else {
                ...
                if (Eagle.getCategoryData(nodeData.inputApplicationType).isApplication){
                    ...
                    errors.push("Attempt to add inputApplication of unsuitable type: " + nodeData.inputApplicationType + ", to node.");
                }
            }
        }
        ...
        if (nodeData.outputAppName !== undefined && nodeData.outputAppName !== ""){
            if (!Eagle.getCategoryData(category).canHaveOutputApplication){
                errors.push("Attempt to add outputApplication to unsuitable node: " + category);
            } else {
                // check applicationType is an application
                if (Eagle.getCategoryData(nodeData.outputApplicationType).isApplication){
                    ...
                    errors.push("Attempt to add outputApplication of unsuitable type: " + nodeData.outputApplicationType + ", to node.");
                }
            }
        }
        ...
        if (nodeData.exitAppName !== undefined && nodeData.exitAppName !== ""){
            if (!Eagle.getCategoryData(category).canHaveExitApplication){
                errors.push("Attempt to add exitApplication to unsuitable node: " + category);
            } else {
                if (Eagle.getCategoryData(nodeData.exitApplicationType).isApplication){
                    ...
                    errors.push("Attempt to add exitApplication of unsuitable type: " + nodeData.exitApplicationType + ", to node.");
                }
            }
        }
        ...
        if (nodeData.application !== undefined && nodeData.application !== ""){
            errors.push("Only found old application type, not new input application type and output application type: " + category);
            ...
            if (!Eagle.getCategoryData(category).canHaveInputApplication){
                ...
                if (Eagle.getCategoryData(category).isApplication){
                    ...
                    errors.push("Attempt to add inputApplication of unsuitable type: " + category + ", to node.");
                }
            }
        }
        ...
        if (typeof nodeData.inputLocalPorts !== 'undefined'){
            for (const inputLocalPort of nodeData.inputLocalPorts){
                if (node.hasInputApplication()){
                    ...
                    errors.push("Can't add inputLocal port " + inputLocalPort.IdText + " to node " + node.getName() + ". No input application.");
                }
            }
        }
        ...
        if (typeof nodeData.outputLocalPorts !== 'undefined'){
            for (const outputLocalPort of nodeData.outputLocalPorts){
                ...
                if (!node.hasOutputApplication() && !node.hasExitApplication()){
                    errors.push("Can't add outputLocal port " + outputLocalPort.IdText + " to node " + node.getName() + ". No output or exit application.");
                }
            }
        }
        ...
        if (typeof nodeData.inputAppFields !== 'undefined'){
            for (const fieldData of nodeData.inputAppFields){
                if (node.hasInputApplication()){
                    ...
                    errors.push("Can't add input app field " + fieldData.text + " to node " + node.getName() + ". No input application.");
                }
            }
        }
        ...
        if (typeof nodeData.outputAppFields !== 'undefined'){
            for (const fieldData of nodeData.outputAppFields){
                if (node.hasOutputApplication()){
                    ...
                    errors.push("Can't add output app field " + fieldData.text + " to node " + node.getName() + ". No output application.");
                }
            }
        }
        ...
        if (typeof nodeData.exitAppFields !== 'undefined'){
            for (const fieldData of nodeData.exitAppFields){
                if (node.hasExitApplication()){
                    ...
                    errors.push("Can't add exit app field " + fieldData.text + " to node " + node.getName() + ". No exit application.");
                }
            }
        }
