...
export class GraphUpdater {
    ...
    static async push(): Promise<void> {
        ...
        if (destRepoValue === "-1") {
            ...
            Eagle.getInstance().repositories().createBranch(this.sourceRepository,newBranchName).then(() => {
                // set the destination repository to be the same as the source repository but with the new branch
                this.destinationRepository = new Repository(this.sourceRepository.service, this.sourceRepository.name, newBranchName, false);
