...
export class Edge {
    ...
    static isValid = (logicalGraph: LogicalGraph, edgeId: string, sourceNodeKey : number, sourcePortId : string, destinationNodeKey : number, destinationPortId : string, dataType: string, loopAware: boolean, closesLoop: boolean, showNotification : boolean, showConsole : boolean, errors: ActionMessage[]) : Eagle.LinkValid => {
        ...
        if (sourcePortId === ""){
            const issue = ActionMessage.Fix(ActionMessage.Level.Error, "source port has no id", function(){Utils.fixNodeFieldIds(sourceNodeKey)}, "Generate ids for ports on source node");
        ...
        if (destinationPortId === ""){
            const issue = ActionMessage.Fix(ActionMessage.Level.Error, "destination port has no id", function(){Utils.fixNodeFieldIds(sourceNodeKey)}, "Generate ids for ports on destination node");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
            return Eagle.LinkValid.Invalid;
        }

        if (sourcePortId === null){
            const issue = ActionMessage.Fix(ActionMessage.Level.Error, "source port id is null", function(){Utils.fixNodeFieldIds(sourceNodeKey)}, "Generate ids for ports on source node");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
            return Eagle.LinkValid.Invalid;
        }

        if (destinationPortId === null){
            const issue = ActionMessage.Fix(ActionMessage.Level.Error, "destination port id is null", function(){Utils.fixNodeFieldIds(sourceNodeKey)}, "Generate ids for ports on destination node");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
            return Eagle.LinkValid.Invalid;
        }
        ...
        if (sourceNode.getCategoryType() === Category.Type.Construct){
            const issue: ActionMessage = ActionMessage.ShowFix(ActionMessage.Level.Error, "Edge (" + edgeId + ") cannot have a source node (" + sourceNode.getName() + ") that is a construct", function(){Utils.showEdge(edgeId)}, function(){Utils.fixMoveEdgeToEmbeddedApplication(logicalGraph, edgeId)}, "Move edge to embedded application");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
        }
        if (destinationNode.getCategoryType() === Category.Type.Construct){
            const issue: ActionMessage = ActionMessage.ShowFix(ActionMessage.Level.Error, "Edge (" + edgeId + ") cannot have a destination node (" + destinationNode.getName() + ") that is a construct", function(){Utils.showEdge(edgeId)}, function(){Utils.fixMoveEdgeToEmbeddedApplication(logicalGraph, edgeId)}, "Move edge to embedded application");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
        }

        // check that we are not connecting two ports within the same node
        if (sourceNodeKey === destinationNodeKey){
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "sourceNodeKey and destinationNodeKey are the same", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
        ...
        if ((sourceNode.getCategory() === Category.Memory && destinationNode.getCategory() === Category.BashShellApp) ||
            ...
            const eagle: Eagle = Eagle.getInstance();
            const issue: ActionMessage = ActionMessage.ShowFix(ActionMessage.Level.Error, "output from Memory Node cannot be input into a BashShellApp or input into a Group Node with a BashShellApp inputApplicationType", function(){Utils.showNode(eagle, Eagle.FileType.Graph, sourceNode.getId())}, function(){Utils.fixNodeCategory(sourceNode, Category.File)}, "Change data component type to File");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, issue, showNotification, showConsole, errors);
        }
        ...
        if (sourcePort === null) {
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "Source port doesn't exist on source node", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
            return Eagle.LinkValid.Impossible;
        }

        // check if destination port was found
        if (destinationPort === null){
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "Destination port doesn't exist on destination node", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
            return Eagle.LinkValid.Impossible;
        }

        // check that we are not connecting a port to itself
        if (sourcePortId === destinationPortId){
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "Source port and destination port are the same", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
            return Eagle.LinkValid.Impossible;
        }

        // check that source is output
        if (!sourcePort.isOutputPort()){
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "Source port is not output port (" + sourcePort.getUsage() + ")", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
            return Eagle.LinkValid.Impossible;
        }

        // check that destination in input
        if (!destinationPort.isInputPort()){
            Edge.isValidLog(edgeId, Eagle.LinkValid.Impossible, ActionMessage.Show(ActionMessage.Level.Error, "Destination port is not input port (" + destinationPort.getUsage() + ")", function(){Utils.showEdge(edgeId);}), showNotification, showConsole, errors);
            return Eagle.LinkValid.Impossible;
        }
        ...
        for (const edge of logicalGraph.getEdges()){
            if (edge.getSrcPortId() === sourcePortId && edge.getDestPortId() === destinationPortId && edge.getId() !== edgeId){
                const x = ActionMessage.ShowFix(ActionMessage.Level.Error, "Edge is a duplicate. Another edge with the same source port and destination port already exists", function(){Utils.showEdge(edgeId);}, function(){Utils.fixDeleteEdge(logicalGraph, edgeId);}, "Delete edge");
                Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, x, showNotification, showConsole, errors);
            }
        }
        ...
        if (sourcePort !== null && !Utils.typesMatch(dataType, sourcePort.getType())){
            ...
            const x = ActionMessage.ShowFix(ActionMessage.Level.Error, "Edge data type (" + dataType + ") does not match start port (" + sourcePort.getDisplayText() + ") data type (" + sourcePort.getType() + ").", function(){Utils.showEdge(edgeId)}, function(){Utils.fixEdgeType(edge, sourcePort.getType());}, "Change edge data type to match source port type");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, x, showNotification, showConsole, errors);
        }
        ...
        if (destinationPort !== null && !Utils.typesMatch(dataType, destinationPort.getType())){
            ...
            const x = ActionMessage.ShowFix(ActionMessage.Level.Error, "Edge data type (" + dataType + ") does not match end port (" + destinationPort.getDisplayText() + ") data type (" + destinationPort.getType() + ").", function(){Utils.showEdge(edgeId)}, function(){Utils.fixEdgeType(edge, destinationPort.getType());}, "Change edge data type to match destination port type");
            Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, x, showNotification, showConsole, errors);
        }
        ...
        if (closesLoop){
            ...
            if (!sourceNode.hasFieldWithDisplayText(Daliuge.FieldName.GROUP_END) || !Utils.asBool(sourceNode.getFieldByDisplayText(Daliuge.FieldName.GROUP_END).getValue())){
                const x = ActionMessage.ShowFix(ActionMessage.Level.Error, "'Closes Loop' Edge (" + edgeId + ") start node (" + sourceNode.getName() + ") does not have '" + Daliuge.FieldName.GROUP_END + "' set to true.", function(){Utils.showEdge(edgeId);}, function(){Utils.fixFieldValue(sourceNode, Daliuge.groupEndField, "true")}, "Set '" + Daliuge.FieldName.GROUP_END + "' to true");
                Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, x, showNotification, showConsole, errors);
            }
            ...
            if (!destinationNode.hasFieldWithDisplayText(Daliuge.FieldName.GROUP_START) || !Utils.asBool(destinationNode.getFieldByDisplayText(Daliuge.FieldName.GROUP_START).getValue())){
                const x = ActionMessage.ShowFix(ActionMessage.Level.Error, "'Closes Loop' Edge (" + edgeId + ") end node (" + destinationNode.getName() + ") does not have '" + Daliuge.FieldName.GROUP_START + "' set to true.", function(){Utils.showEdge(edgeId);}, function(){Utils.fixFieldValue(destinationNode, Daliuge.groupStartField, "true")}, "Set '" + Daliuge.FieldName.GROUP_START + "' to true");
                Edge.isValidLog(edgeId, Eagle.LinkValid.Invalid, x, showNotification, showConsole, errors);
            }
        }
