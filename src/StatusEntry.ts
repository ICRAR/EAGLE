...
export class StatusEntry {
    ...
    static getStatusEntries() : StatusEntry[] {
        return [
            ...
            new StatusEntry(KeyboardShortcut.idToText('open_graph_attributes_configuration_table_modal', true),' display active graph config.', Eagle.getInstance().logicalGraph().fileInfo().name != "" && Eagle.getInstance().selectedObjects().length === 0),
            ...
            //node is selected
            new StatusEntry(KeyboardShortcut.idToText('open_parameter_table_modal', true),' open fields table.', Eagle.getInstance().selectedNode() != null && Setting.findValue(Setting.ALLOW_GRAPH_EDITING)),
            ...
            //always
            new StatusEntry(KeyboardShortcut.idToText('open_graph_configurations_table_modal', true), ' display all graph configs.', true),
        ];
    }
}