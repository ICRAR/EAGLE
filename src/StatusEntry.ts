...
export class StatusEntry {
    ...
    // TODO: change from function to a set variable 
    static getStatusEntries() : StatusEntry[] {
        return [
            ...
            new StatusEntry(KeyboardShortcut.idToKeysText('duplicate_selection', true),' duplicate selected objects.', Eagle.getInstance().selectedObjects().length > 0),
            new StatusEntry(KeyboardShortcut.idToKeysText('delete_selection', true),' delete selected objects.', Eagle.getInstance().selectedObjects().length > 0),
            //node is selected
