...
.setPreFunction(function(){const targetFunc = TutorialSystem.activeTutCurrentStep.getTargetFunc(); if (targetFunc === null){return;} targetFunc().parent().addClass('forceShow');$("#navTutorials").find('.dropDropDown').addClass('forceShow')}) //keeping the navbar graph dropdown open and showing the contents of the tutorials drop down
...
.setBackPreFunction(function(){const targetFunc = TutorialSystem.activeTutCurrentStep.getTargetFunc(); if (targetFunc === null){return;} targetFunc().parent().addClass('forceShow');}) //hide the modal in case it has been opened by the user
