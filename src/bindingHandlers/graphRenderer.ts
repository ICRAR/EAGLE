...
function render(graph: LogicalGraph, elementId : string, eagle : Eagle){
    ...
    const COLLAPSED_NODE_WIDTH : number = 128;
    const COLLAPSED_NODE_HEIGHT : number = 128;
    const DATA_COMPONENT_NODE_WIDTH : number = 48;
    const DATA_COMPONENT_NODE_HEIGHT : number = 48;
    ...
    var inputPortGroups = nodes.append("g")
                                ...
                                //.attr("transform", getInputPortsPosition)
                                .attr("transform", function(node : Node){return buildTranslation(REAL_TO_DISPLAY_SCALE(2), REAL_TO_DISPLAY_SCALE(24))})
    ...
    var inputLocalPortGroups = nodes.append("g")
                                ...
                                .attr("transform", function(node : Node){return buildTranslation(REAL_TO_DISPLAY_SCALE(getWidth(node)-2), REAL_TO_DISPLAY_SCALE((node.getOutputPorts().length + 1) * 24))})
                                .style("display", getPortsDisplay);
    ...
    var outputPortGroups = nodes.append("g")
                                ...
                                .attr("transform", function(node : Node){return buildTranslation(REAL_TO_DISPLAY_SCALE(getWidth(node)-2), REAL_TO_DISPLAY_SCALE(24))})
                                .style("display", getPortsDisplay);
    ...
    var outputLocalPortGroups = nodes.append("g")
                                ...
                                .attr("transform", function(node : Node){return buildTranslation(REAL_TO_DISPLAY_SCALE(2), REAL_TO_DISPLAY_SCALE((node.getInputPorts().length + 1) * 24))})
                                .style("display", getPortsDisplay);
    ...
    var portDragHandler = d3.drag()
                            .on("start", function (port : Port) {
                                //console.log("drag start", port.getId(), port.getNodeId());
                                isDraggingPort = true;
                            ...
                            .on("end", function(port : Port){
                                ...
                                if (destinationPortId !== null){
                                    ...
                                    var linkValid : Eagle.LinkValid = Edge.isValid(graph, sourceNodeKey, sourcePortId, destinationNodeKey, destinationPortId);
    ...
    function tick(){
        ...
        svgContainer.selectAll("g.node rect:not(.header-background):not(.resize-control):not(.shrink-button):not(.collapse-button):not(.expand-button)")
                                .data(nodeData)
    ...
    function getWidth(node : Node) : number {
        ...
        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return DATA_COMPONENT_NODE_WIDTH;
        }
    ...
    function getHeight(node : Node) : number {
        ...
        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return DATA_COMPONENT_NODE_HEIGHT;
        }

        var leftHeight = ((1 + node.getInputPorts().length + node.getOutputLocalPorts().length) * 24) + 24;
        var rightHeight = ((1 + node.getOutputPorts().length + node.getInputLocalPorts().length) * 24) + 24;
    ...
    function getHeadingPositionX(node : Node) : number {
        ...
        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return DATA_COMPONENT_NODE_WIDTH / 2;
        }
    ...
    function getHeadingPositionY(node : Node) : number {
        if (node.isGroup() && node.isCollapsed()){
            return COLLAPSED_NODE_HEIGHT / 2;
        }

        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return (3 * DATA_COMPONENT_NODE_HEIGHT / 2);
        }
    ...
    function edgeGetX1(edge: Edge) : number {
        ...
        if (node.isCollapsed()){
            return node.getPosition().x + COLLAPSED_NODE_WIDTH;
        }

        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return node.getPosition().x + (DATA_COMPONENT_NODE_WIDTH/2);
        }
        ...
        if (collapsedAncestor !== null){
            return collapsedAncestor.getPosition().x + COLLAPSED_NODE_WIDTH;
        }

        return node.findNodePortPosition(edge.getSrcPortId()).x;
    }
    ...
    function edgeGetY1(edge: Edge) : number {
        ...
        if (node.getCategoryType() === Eagle.CategoryType.Data && !node.isShowPorts()){
            return node.getPosition().y + (DATA_COMPONENT_NODE_HEIGHT / 2);
        }
        ...
        return node.findNodePortPosition(edge.getSrcPortId()).y;
    }
    ...
    function edgeGetX2(edge: Edge) : number {
        ...
        return node.findNodePortPosition(edge.getDestPortId()).x;
    }
    ...
    function edgeGetY2(edge: Edge) : number {
        ...
        return node.findNodePortPosition(edge.getDestPortId()).y;
    }

    function edgeGetStrokeColor(edge: Edge, index: number) : string {
        var linkValid : Eagle.LinkValid = Edge.isValid(graph, edge.getSrcNodeKey(), edge.getSrcPortId(), edge.getDestNodeKey(), edge.getDestPortId(), false);
    ...
    function shrinkOnClick(node : Node, index : number){
        ...
        for (var i = 0 ; i < nodeData.length ; i++){
            ...
            if (n.getParentKey() === node.getKey()){
                ...
                if (n.getPosition().x + getWidth(n) > maxX){
                    maxX = n.getPosition().x + getWidth(n);
                }
                if (n.getPosition().y + getHeight(n) > maxY){
                    maxY = n.getPosition().y + getHeight(n);
                }
            }
        }
    ...
    function getInputPortsPosition(node : Node) : string {
        if (false && node.isGroup()){
            return buildTranslation(2, -8 - 24 * node.getInputPorts().length);
        } else {
            return buildTranslation(2, 24);
        }
    }

    function getOutputPortsPosition(node : Node) : string {
        if (false && node.isGroup()){
            return buildTranslation(getWidth(node)-2, -8 - 24 * node.getOutputPorts().length);
        } else {
            return buildTranslation(getWidth(node)-2, 24);
        }
    }
    ...
    function mouseEnterPort(port : Port) : void {
        ...
        isDraggingPortValid = Edge.isValid(graph, sourceNodeKey, sourcePortId, destinationNodeKey, destinationPortId);
    }
