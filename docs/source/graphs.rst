...
The |daliuge| execution engine requires that the processing workflow is described by a Directed Acyclic Graph (DAG) where the nodes
denote both task (application DROPs) and data (data DROPs). The edges denote
execution dependencies or event passing between DROPs (note that in contrary to MPI, events in |daliuge| are not carrying data artefacts). The |daliuge| system distinguishes between 4 different, tightly linked graph types. A single processing workflow will transition from one type to the next, during its lifecycle. The four graph types are:
...
EAGLE deals with the first two, the *translator* produces the latter two while the *engine* only deals with the last, which contains all the required information to execute the processing workflow. This document, since it is part of the EAGLE documentation, only describes the |lgt| and the |lg|

Logical Graphs
^^^^^^^^^^^^^^

A |lgt| as well as a |lg| is a compact representation of the logical operations in a processing
workflow without being concerned of underlying hardware resources or constraints. The compactness is achieved by high level *constructs*, which allow the |lg| developer to pack highly parallel sections of a workflow into a single block and essentially describing everything that could be executed in parallel just once. 

|lgt| vs |lg|
"""""""""""""
The difference between a |lgt| and a |lg| seems subtle, but is quite significant. A |lgt| can be seen as a blueprint describing a certain kind of data reduction or a data reduction capability. For instance a spectral line imaging workflow or a mosaicing workflow are described by |lgts|, where the specific configuration parameters for each of the individual operations in the workflow are still undefined or just the default ones. For many real scientific projects it will be required to control at least some of these parameters in order to achieve the anticipated results. Changing these parameters does not change the overall logical representation of the workflow at all. As an example assume a flexible frequency averaging operation, allowing the user to specify the number of channels to be averaged into a single one. Changing that number does not change the structure of the |lg|, but it will certainly change the final results as well as the resulting |pg|, which will eventually be executed.

The use case for |lgts| and |lgs| is that observatory staff would develop certain high level data reduction capabilities and offer those capabilities to the scientists applying for time on the telescope. When specifying the details of the proposal the scientists will then also be able to tailor the required data reduction to their specific needs. In the |dlg| world, this translates to the observatory staff developing |lgts| and the scientists to select one of them and specify the required parameter values, which turns it into a |lg|. EAGLE supports the development of |lgts| and also the parameterisation step into |lgs|.
...
A |lg| consists of 5 basic or abstract classes of graph nodes:
...
The nodes used to construct |lgts| are instances of one of these classes. Such instances represent concrete entities, like a specific algorithm, or a specific data file. In order to construct a useful and executable |lgt|, EAGLE (and the user) needs a collection of such concrete nodes. Such a collection is called a *Palette*. The super-collection of all available concrete nodes is called *Pool*. It is anticipated that the *Pool* will grow quite large fairly quickly and that is the reason to allow people to work with subsets (*palettes*) instead.

*Application nodes*
+++++++++++++++++++

*Application nodes* represent operations or individual algorithms turning one or more input data nodes into one or more output data nodes. An application node not using and producing any data is just producing heat and is pretty much useless for any practical workflow. In fact the *Data Activated* part of the |dlg| name hints at the fundamental assumption of the whole system, that it is the *data* what is really important, not the operations. Any workflow starts with some data on input and produces some data as output. Even a simulation needs input data, which might only consist of configuration parameters. The operations are merely the means to an end.

*Data nodes*
++++++++++++

*Data nodes* represent data artefacts. There are a few derived, also abstract, sub-types of *Data nodes* like *File*, *Memory* or *S3* available. Concrete data nodes will define at least the highest level of content type, e.g. a mime-type.

*Flow Construct nodes*
++++++++++++++++++++++

form the "skeleton" of the |lg|, and determine
the final structure of the |pg| to be generated. |daliuge| currently supports
the following flow constructs:

  * **Scatter** indicates data parallelism. Constructs inside a *Scatter* construct represent a group of components consuming a single data partition within the enclosing *Scatter*. A useful property of *Scatter* is ``num_of_copies``. In the example in :numref:`graphs.figs.scatter`, if the ``num_of_copies`` for ``Scatter1`` and ``Scatter2`` are 5 and 4 respectively, the generated |pg| will have in total 20 ``Data1``/``Component1``/``Data3`` DROPs, but only 5 DROPs for the construct ``Component 5``, which is inside the ``Scatter1`` construct but outside ``Scatter2``.

  * **Gather** indicates data barriers. Constructs inside a *Gather* represent a group of components consuming a sequence of data partitions as a whole. *Gather* has a ``num_of_inputs`` property, which represents the *Gather* "width", stating how many partitions each *Gather* instance (translated into a ``BarrierAppDROP``, see :ref:`drop.component.iface`) can handle. This in turn is used by |daliuge| to determine how many *Gather* instances should be generated in the |pg|. *Gather* sometimes can be used in conjunction with *Group By* (see middle-right in :numref:`graphs.figs.scatter`), in which case, data held in a sequence of groups are processed together by components enclosed by *Gather*.

  * **Group By** indicates data resorting (e.g. `corner turning <https://mnras.oxfordjournals.org/content/410/3/2075.full>`_ in radio astronomy). The semantic is analogous to the ``GROUP BY`` construct used in SQL statement for relational  databases, but applied to data DROPs. The current |daliuge| prototype requires that *Group By* is used in conjunction with a nested *Scatter* such that data DROPs that are originally sorted in the order of ``[outer_partition_id][inner_partition_id]`` are resorted as ``[inner_partition_id][outer_partition_id]``. In terms of parallelism, *Group By* is comparable to the `"static" MapReduce <http://openmymind.net/2011/1/20/Understanding-Map-Reduce/>`_, where the keys used by all Reducers are known a priori.

  * **Loop** indicates iterations. Constructs inside a *Loop* represent a group of components and data that will be repeatedly executed / produced for a fixed number of times. Given the basic DROP principle of "writing once, read many times", the current |daliuge| prototype does not support dynamic branch condition for *Loop*. Instead, each *Loop* construct has a property named ``num_of_iterations`` that must be determined at |lg| development time, and that determines the number of times the loop is "unrolled". In other words, a ``num_of_iterations`` number of DROPs for each construct inside a *Loop* will be statically generated in the |pg|. An example is shown in :numref:`graphs.figs.loop`.
