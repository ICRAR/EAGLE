export async function enableMouseCursor(page){
  await page.evaluate(() => {
    ...
    arrowContainer.style.cssText = 'position: absolute; top: 0; left: 0; width: 35px; height:35px; z-index: 9999; transition:left 1s ease-in-out, top 1s ease-in-out;';
...
export async function moveMouseCursor(page, targetElement){
  ...
  await page.evaluate(({newX, newY}) => {
    //getting the fake cursor element
